...
class Animation:
  def __init__(self, map, schedule):
    ...
    # draw starts
    for d, i in zip(map["agents"], range(0, len(map["agents"]))):
      self.patches.append(Rectangle((d["start"][0] - 0.25, d["start"][1] - 0.25), 0.5, 0.5, facecolor=Colors[3], edgecolor='black', alpha=0.5))
    for d, i in zip(map["agents"], range(0, len(map["agents"]))):
      name = d["name"]
      self.agents[name] = Circle((d["start"][0], d["start"][1]), 0.45, facecolor=Colors[0], edgecolor='black')
